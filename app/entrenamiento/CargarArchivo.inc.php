...
if (empty($_FILES["file1"])) {
	...
	$carpeta = "../../assets/entrenamiento/cover";//carpeta donde se suben los archivos
	
	$fileName = str_replace(' ', '-', $fileName);

	$directorio = DIRECTORIO_RAIZ."/assets/entrenamiento/cover/";//se crea la variable para la ruta de la carpeta
	...
	if(move_uploaded_file($fileTmpLoc, $carpeta."/".$nombreEmpresa.date("d-m-Y").$string_aleatorio.utf8_decode($fileName))){		
		...
		<img src="<?php echo RUTA_RUTINA_EJERCICIOS_COVER.$nombreEmpresa.date("d-m-Y").$string_aleatorio."$fileName";?>" class="imagen-3">
